...
local function InitAPI()
    ...
    HamingwaysHunterTools_API.GetBaseWeaponSpeed = function() return baseWeaponSpeed end
    HamingwaysHunterTools_API.GetHasteMultiplier = function() return hasteMultiplier end
...
-- Scan base weapon speed from tooltip (like Quiver)
local scanTooltip = nil
local function ScanBaseWeaponSpeed()
    if not scanTooltip then
        scanTooltip = CreateFrame("GameTooltip", "HHT_WeaponSpeedScan", nil, "GameTooltipTemplate")
        scanTooltip:SetOwner(UIParent, "ANCHOR_NONE")
    end
    
    scanTooltip:ClearLines()
    scanTooltip:SetInventoryItem("player", 18)  -- Ranged weapon slot
    
    -- Scan tooltip for "Speed X.XX"
    for i = 1, scanTooltip:NumLines() do
        local rightText = getglobal("HHT_WeaponSpeedScanTextRight" .. i)
        if rightText then
            local text = rightText:GetText()
            if text then
                local _, _, speed = string.find(text, "(%d+%.%d+)")
                if speed then
                    baseWeaponSpeed = tonumber(speed)
                    return
                end
            end
        end
    ...
    -- Fallback: Use current speed as base
    local currentSpeed = UnitRangedDamage("player")
    if currentSpeed and currentSpeed > 0 then
        baseWeaponSpeed = currentSpeed
    end
end
...
local function UpdateWeaponSpeed()
    ...
    if speed and speed > 0 then
        weaponSpeed = speed
        -- Calculate haste multiplier (Lua 5.0: check for nil before comparing)
        if baseWeaponSpeed and baseWeaponSpeed > 0 then
            hasteMultiplier = weaponSpeed / baseWeaponSpeed
        end
    end
end
...
local function UpdateBarShoot(now)
    ...
    if remainingShoot > 0 then
        ...
        barFrame:SetVertexColor(db.aimingColor.r, db.aimingColor.g, db.aimingColor.b, BAR_ALPHA)
...
local function UpdateBarReload(now)
    ...
    if remainingReload > 0 then
        ...
        barFrame:SetWidth(width)
        barFrame:SetVertexColor(db.reloadColor.r, db.reloadColor.g, db.reloadColor.b, BAR_ALPHA)
        if db.showTimerText and now - textCache.lastUpdate < textCache.interval then
...
local function UpdateBarMelee(now)
    ...
    if remainingSwing > 0 then
        ...
        local width = math.max(1, maxBarWidth * percentElapsed)
        barFrame:SetWidth(width)
        barFrame:SetVertexColor(db.meleeColor.r, db.meleeColor.g, db.meleeColor.b, BAR_ALPHA)
    else
        -- Ready for next swing (100% full, change color to indicate ready)
        barFrame:SetWidth(maxBarWidth)
        barFrame:SetVertexColor(1.0, 0.8, 0.8, BAR_ALPHA)  -- Lighter color when ready
    end
end
...
local function ApplyFrameSettings()
    ...
    if castFrame then
        ...
        if castBarFrame then
            ...
            -- Update bar texture
            castBarFrame:SetTexture(cfg.barStyle or "Interface\\TargetingFrame\\UI-StatusBar")
            -- Set color AFTER SetTexture
            castBarFrame:SetVertexColor(cfg.castColor.r, cfg.castColor.g, cfg.castColor.b, BAR_ALPHA)
        end
    end
...
local function ShowConfigPreview()
    ...
    if castFrame and castBarFrame and castBarText and castBarTextTime then
        castFrame:Show()
...
local function HideConfigPreview()
    ...
    if castFrame and not isCasting then
        castFrame:Hide()
    end
...
local function CreateConfigFrame()
    ...
    -- Create tabs (split into 3 rows: 4+4+2)
    local tabNames = {"AutoShot", "Castbar", "Buffs", "Ammunition", "Statistik", "Pet Feeder", "Melee", "Warnings", "Tranq", "Appearance"}
    local tabWidthRow1 = (320 - 32) / 4  -- First 4 tabs
    local tabWidthRow2 = (320 - 32) / 4  -- Next 4 tabs
    local tabWidthRow3 = (320 - 32) / 2  -- Last 2 tabs
    ...
    frame:SetScript("OnUpdate", function()
        ...
        if db.showAutoShotTimer and useMeleeTimer then
            if db.showOnlyInCombat and not UnitAffectingCombat("player") then
                ...
                if barFrame then
                    barFrame:SetWidth(1)
                    barFrame:SetVertexColor(0.3, 0.3, 0.3, 0.5)
                    barTextTotal:SetText("")
...
local function CreatePetFeederFrame()
    ...
    petFeedFrame:SetBackdrop(nil)  -- Main frame is transparent (only for dragging)
...
local function CreateMainFrame()
    ...
    local cfg = GetConfig()
    barFrame = frame:CreateTexture(nil, "BORDER")
...
local function HandleInventoryChanged()
    ScanBaseWeaponSpeed()  -- Rescan base speed when equipment changes
    UpdateWeaponSpeed()
    UpdateAmmoDisplay()
end
...
SlashCmdList["HamingwaysHunterTools"] = function(msgArg)
    ...
    elseif msgLower == "resetwarnings" then
        ...
        print("HHT: /HamingwaysHunterTools reset | lock | unlock | config | test | debug | stats | resetwarnings")
    end
end
