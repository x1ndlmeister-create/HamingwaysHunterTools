...
function HHT_PetFeeder_UpdateDisplay(forceUpdate)
    ...
    -- Reset display cache on forced update to ensure texture is refreshed
    if forceUpdate then
        lastDisplayedFood = nil
        lastDisplayedCount = nil
        lastDisplayedHappiness = nil
        lastDisplayUpdate = 0  -- Allow immediate subsequent updates
        lastPetNameCheck = 0  -- Refresh pet name cache
    else
    ...
    -- Load selected food for current pet (or clear if no saved food)
    if petName then
        if HamingwaysHunterToolsDB.selectedFood and HamingwaysHunterToolsDB.selectedFood[petName] then
            selectedFood = HamingwaysHunterToolsDB.selectedFood[petName]
        else
            selectedFood = nil  -- No saved food for this pet
        end
    end
