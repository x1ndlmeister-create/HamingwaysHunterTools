...
-- Version: 1.1.0 - Lock and Load / Experimental Ammo integration via HHT_ProcState API
-- Author: Copilot
...
local cachedAimedID, cachedAimedName = nil, nil
...
local function CacheSpellIDs()
    ...
    while true do
        ...
        elseif (name == "Aimed Shot" or name == "Gezielter Schuss") and not cachedAimedID then
            cachedAimedID = i
            cachedAimedName = name
        elseif name == "Rapid Fire" and not cachedRapidFireID then
            cachedRapidFireID = i
        end
        
        if cachedSteadyID and cachedMultiID and cachedAimedID and cachedRapidFireID then
            break
        end
...
local function TryBurst()
    if not LazyHuntDB.burstEnabled then
        doBurst = false  -- ensure stale flag never fires when burst is disabled
        return false
    end
...
rotationFrame:SetScript("OnUpdate", function()
    ...
    if currentStep == STEP_WORK then
        ...
        if not isShooting or not UnitExists("target") or UnitIsDead("target") then
                ...
                doBurst = false  -- clear burst flag when out of combat / no target
                return
        end
        --isShooting_old = isShooting
        ...
        if isInRedPhase then
            ...
            elseif currentRotaStep == ROTA_DO_MULTIAFTERSTEADY then
                ...
                -- When LnL is active we cast Aimed Shot (1.0s with LnL, 2.0s base) instead of Steady Shot
                -- Use HHT haste multiplier for accurate cast time
                local lnlActive = HHT_ProcState_LnL and HHT_ProcState_LnL() or false
                local nextShotBaseCast
                if lnlActive then
                    nextShotBaseCast = 1.0  -- Aimed Shot cast time with Lock and Load
                else
                    nextShotBaseCast = 1.0 * (HamingwaysHunterTools_API.GetBaseWeaponSpeed() / 2.0)
                end
                local steadyCastDuration = nextShotBaseCast * HamingwaysHunterTools_API.GetHasteMultiplier()
...
function LazyHunt_ExecuteAction()
	...
	-- we should do steady (or Aimed Shot if Lock and Load is active)
    elseif LazyHunt_NextAction == "STEADY" then
        local lnlActive = HHT_ProcState_LnL and HHT_ProcState_LnL() or false
        if lnlActive and cachedAimedID then
            local aimedCastTime = 1.0 * HamingwaysHunterTools_API.GetHasteMultiplier()  -- 1s with LnL, hasted
            HamingwaysHunterTools_API.NotifyCast(cachedAimedName, aimedCastTime)
            CastSpellByName(cachedAimedName)
        else
    ...
    elseif LazyHunt_NextAction == "MULTI" then
        -- Calculate Multi-Shot cast time with haste (base 0.5s * haste multiplier)
        local multiCastTime = 0.5
        if HamingwaysHunterTools_API and HamingwaysHunterTools_API.GetHasteMultiplier then
            local hasteMultiplier = HamingwaysHunterTools_API.GetHasteMultiplier()
            if hasteMultiplier and hasteMultiplier < 1.0 then
                multiCastTime = 0.5 * hasteMultiplier
            end
        end
        HamingwaysHunterTools_API.NotifyCast(cachedMultiName, multiCastTime)
